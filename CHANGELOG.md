...
- 修复限流值非法时返回 `HTTP 500 integer divide by zero` 的问题。`api/limit.go` 旧逻辑只校验了配置值里有没有 `/`，没校验次数与时长：`0/1h`、`abc/3h`（`gconv.Int("abc")` 为 `0`）都会由于 `rate.NewLimiter` 内部 `per/limit` 的整数除法而 panic；`-1/1h` 虽不 panic，但会让该模型恒返回 429、`please wait 0 seconds`，实际不可用。现新增 `parseModelRate()` 统一校验「次数为正整数、时长为可解析的正 duration」，非法时回退到兜底限流 `40/3h` 并输出 `WARN` 日志（与原先「值里没有 `/` 就回退」的既有行为保持一致，不再把配置写错升级成 500）。模型与 `DEFAULT` 都未配置时属于常态（例如 Docker 部署未挂载 `config.yaml` 且未设置同名环境变量），这种情况静默回退、不打告警。
